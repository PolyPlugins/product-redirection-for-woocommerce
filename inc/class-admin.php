...
if (!class_exists('ADMIN_PRFW')) {
  ...
  $admin = new ADMIN_PRFW;
  $admin->init();

} else {
  PRODUCT_REDIRECTION_FOR_WOOCOMMMERCE_PP::admin_notice("Product Redirection for WooCommerce Admin handling is not running because the 'ADMIN_PRFW' class already exists. A conflict may be happening with another plugin. <a href='https://wordpress.org/support/plugin/product-redirection-for-woocommerce/' target='_blank'>Get Support</a>");
}